...
KMS_COMPOSITOR_SOURCES := displaylink_kms_renderer.c display_renderer.c overlay_text.c breezy_overlay.c usbip.c udl_device.c udl_runtime.c server.c display_placement.c breezy_imu.c usb_gadget.c link_services.c $(COMPAT_SOURCES)
...
$(KMS_RENDERER_TARGET): $(KMS_COMPOSITOR_SOURCES) $(ZEROKVM_LIB) $(JS_BUNDLE) $(QUICKJS_OBJS)
	$(CC) $(CPPFLAGS) $(CFLAGS) $(PROFILE_CFLAGS) $(DRM_CFLAGS) $(JSON_C_CFLAGS) -I$(QUICKJS_DIR) -I. -pthread \
		-o $@ $(KMS_COMPOSITOR_SOURCES) $(QUICKJS_OBJS) \
		$(DRM_LIBS) $(GBM_LIBS) $(EGL_LIBS) $(GLES2_LIBS) $(JSON_C_LIBS) -lm $(LDFLAGS) $(LDLIBS)

$(JS_BUNDLE): $(JS_SHARED_DIR)/math.js $(JS_SHARED_DIR)/displayPlacement.js tools/gen_js_bundle.py
	$(PYTHON3) tools/gen_js_bundle.py $(JS_SHARED_DIR)/math.js $(JS_SHARED_DIR)/displayPlacement.js > $@.tmp
