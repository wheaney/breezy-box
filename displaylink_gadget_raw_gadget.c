...
static void usage(const char *argv0)
{
	fprintf(stderr,
		...
		"  --width PIXELS          Display width (drives EDID, decode buffers, and SDL window; default: 1920)\n"
		"  --height PIXELS         Display height (drives EDID, decode buffers, and SDL window; default: 1080)\n"
		"  --decode-width PIXELS   Deprecated alias for --width\n"
		"  --decode-height PIXELS  Deprecated alias for --height\n"
...
static int validate_options(const struct options *opts)
{
	...
	if (opts->decode_width == 0u || opts->decode_height == 0u) {
		fprintf(stderr, "width/height must be at least 1x1\n");
		return -1;
	}
...
static void build_preferred_dtd_60hz_rb(uint8_t dtd[18], uint32_t width, uint32_t height)
{
	...
	const uint32_t pixel_clock_10khz = (uint32_t)((((uint64_t)h_total * (uint64_t)v_total * 60u) + 5000u) / 10000u);
...
static void build_default_edid(uint8_t edid[128], const char *monitor_name, uint32_t width, uint32_t height)
{
	uint32_t sum = 0u;
	size_t index;
	uint8_t dtd[18];
	...
	build_preferred_dtd_60hz_rb(dtd, width_clamped, height_clamped);
